    ...
    <div className="min-h-screen bg-[#060a13] flex font-sans">
      ...
      <aside className={`fixed lg:sticky top-0 left-0 h-screen z-50 flex flex-col transition-all duration-300 ease-out ${sideWidth}
          ...
          <div className="relative flex items-center gap-2.5">
            ...
            {!collapsed && (
              <div className="animate-fadeIn">
                ...
                <p className="text-[10px] text-slate-600 font-medium">Matcher v2.0</p>
              </div>
            )}
          </div>
        </div>
        ...
        <nav className={`flex-1 ${collapsed ? "px-2" : "px-3"} py-4 space-y-1 overflow-y-auto`}>
          {!collapsed && <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-600 px-3 mb-2">Navigation</p>}
          {navItems.map((item) => {
        ...
        <div className="hidden lg:flex justify-center py-3 border-t border-white/[0.04]">
          <button onClick={() => setCollapsed(!collapsed)}
            className="p-2 rounded-lg text-slate-600 hover:text-slate-400 hover:bg-white/[0.03] transition-all">
          ...
          <div className="px-3 py-3 border-t border-white/[0.04] flex-shrink-0">
            <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-xl" style={{ background: 'rgba(6,10,19,0.6)', border: '1px solid rgba(255,255,255,0.03)' }}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10px] text-slate-600 font-medium">{user.role === "clinical_researcher" ? "Researcher" : "Doctor"}</p>
              </div>
            </div>
            <button type="button" onClick={onLogout}
              className="mt-2 w-full flex items-center justify-center gap-1.5 text-[11px] font-medium text-slate-600 hover:text-red-400 py-2 rounded-lg hover:bg-red-400/5 transition-all">
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-30 h-16 flex items-center justify-between px-6 border-b border-white/[0.04]"
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setMobileOpen(true)} className="lg:hidden p-2 rounded-lg text-slate-500 hover:text-slate-300 hover:bg-white/[0.03]">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-2 rounded-xl px-3 py-2"
              ...
              <input placeholder="Search..." className="bg-transparent text-sm text-slate-300 placeholder:text-slate-600 outline-none w-40" />
              <kbd className="text-[9px] font-sans font-semibold text-slate-600 bg-white/[0.03] border border-white/[0.06] px-1.5 py-0.5 rounded">⌘K</kbd>
            </div>
            ...
            <button className="p-2 rounded-xl text-slate-500 hover:text-slate-300 hover:bg-white/[0.03] transition-all">
