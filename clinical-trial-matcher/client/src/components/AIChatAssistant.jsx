  ...
  return (
    <div className="rounded-2xl p-6" style={{ background: 'rgba(10,15,28,0.6)', border: '1px solid rgba(255,255,255,0.04)' }}>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-600">AI Chat Assistant</p>
      ...
      <form onSubmit={(e) => { e.preventDefault(); askQuestion(question.trim()); }} className="flex gap-2">
        <input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Ask about the selected trial match"
          className="flex-1 rounded-xl px-4 py-2.5 text-sm bg-white/[0.02] border border-white/[0.05] text-slate-200 placeholder:text-slate-700 outline-none focus:border-teal-500/30 transition-all" />
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {SUGGESTED.map((q) => (
          <button key={q} type="button" onClick={() => { setQuestion(q); askQuestion(q); }} disabled={loading}
            className="rounded-full px-3 py-1.5 text-[11px] font-medium text-slate-600 disabled:opacity-30 transition-all flex items-center gap-1.5 hover:text-teal-400 hover:border-teal-400/20"
      ...
      <div className="mt-5 space-y-3">
        {messages.length === 0 ? (
          <div className="rounded-xl p-8 text-center" style={{ background: 'rgba(6,10,19,0.4)', border: '1px dashed rgba(255,255,255,0.04)' }}>
            <Bot className="w-10 h-10 text-slate-700/40 mx-auto mb-3" />
            <p className="text-sm text-slate-500">No conversation yet</p>
            <p className="text-xs text-slate-600 mt-1">Select a trial and ask a question to begin.</p>
          </div>
        ) : (
          messages.map((item, i) => (
